      ...
      <Card>
        ...
        <CardContent className="space-y-4">
            ...
            {coalitionParties.map((p) => (
              <div key={p.id} className="flex justify-between border rounded-md p-2 text-sm">
                ...
                <span className="font-mono">
                  {p.seats}議席 ・距離{' '}
                  {rulingParty ? ideologyDistance(rulingParty.ideology, p.ideology).toFixed(2) : '―'}
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="border rounded-md p-3 text-sm bg-muted/40">
            ...
            <div>連立健全度初期値: {Math.round(state.coalitionHealth ?? 0)}</div>
          </div>

          <div className="text-sm">
            {isPlayerInCoalition ? (
              <span className="text-green-700">あなたの党は与党 (連立) として政権に参加します。</span>
            ) : (
